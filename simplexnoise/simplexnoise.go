...
func rescaleAndDraw(noise []float32,min, max float32, gradient []color, pixels []byte) {		//rescale noise --> turn in to byte --> set pixel array with that byte
	...
	for i := range noise {									//going thru slice of noise
		...
		p := i * 4
		pixels [p] = c.r									//set those byte in to pixels slice
		pixels [p + 1] = c.g
		pixels [p+ 2] = c.b
	}
}

func turbulence(x, y, frequency, lactunarity, gain float32, octaves int) float32 {
	var sum float32
	amptitude := float32(1)

	for i := 0; i < octaves; i++ {
		f := snoise2(x * frequency, y * frequency) * amptitude
		if f < 0 {
			f = -1.0 * f
		}
		sum += f
...
func fbm2(x, y, frequensy, lacunarity, gain float32, octaves int)float32{  //fractal brownian motion func, (lacunarity - reit of changing frequency; gain - rait of changen amptitude; octaves - how many iterations )
	 var sum float32
	 amptitude := float32(1.0)
	 for i := 0; i < octaves; i++ {
		 sum += snoise2(x * frequensy, y * frequensy) * amptitude
		 frequensy = frequensy * lacunarity
		 amptitude = amptitude * gain
	 }
	 return sum
}

func makeNoise(pixels []byte, frequency, lactunarity, gain float32, octaves, w, h int){
	var mutex = & sync.Mutex{}
	startTime := time.Now()									// to time how long takes to execute untill elapsedTime
	noise := make([]float32, winWidth*winHeight)			//for collecting all float 32 noise
	fmt.Println("freq: ", frequency, "lac: ", lactunarity, "gain: ", gain, "octaves: ", octaves)  //so we can see in terminal how values changed

	min := float32(9999.0)
	max := float32(-9999.0)

	numRoutines := runtime.NumCPU()						//to know how much logical cpu's u can use
	var wg sync.WaitGroup								//creating wait group for gorutines
	wg.Add(numRoutines)									// add to wait list how much gorutines we can execute

	batchSize := len(noise) / numRoutines				//"job pool" for our gorutines 

	for i := 0; i < numRoutines; i++ {					//creating loop where gorutens going to work
		go func(i int) {
			defer wg.Done()								//when gorutine done, it calls wait group and free one more spot
			start := i * batchSize
			...
			if noise[j] < min || noise[j]> max{
				mutex.Lock()
				if noise[j] < min{
				min = noise[j]							//help us tu find min noise going thru slice
			}else if noise[j] > max {
				max = noise[j]							//help us to find max noise going thru slice
			}
			mutex.Unlock()
		}
		}
	...
	wg.Wait()											//waiting here until gorutines finished
	elapsedTime := time.Since(startTime).Seconds() * 1000 // how much time passed in ms
	fmt.Println(elapsedTime)
	gradient := getDualGradient(color{0,0,175}, color{80, 160, 244}, color{12,192,75}, color{255,255,255})		//gradient between those colors
...
keyState := sdl.GetKeyboardState()				//changing keystate - changing values
...
for {
	...
	mult := 1
	if keyState[sdl.SCANCODE_LSHIFT] !=0 || keyState [sdl.SCANCODE_RSHIFT] != 0 {	//with shift changing multiplayer to see how output would react
		mult = -1
		makeNoise(pixels, frequency, lac, gain, octaves, winWidth, winHeight) 	// to redraw each time values changed
	}
	if keyState[sdl. SCANCODE_O] != 0 {						// "!= 0" - means button pressed (in state 1)  button "o"
		octaves = octaves + 1*mult
		makeNoise(pixels, frequency, lac, gain, octaves, winWidth, winHeight)
	}
	if keyState[sdl.SCANCODE_F] != 0 {						//button "f" to effect frequency
		frequency = frequency + 0.001 * float32(mult)		//dont forget convert mult(int) in to float
		makeNoise(pixels, frequency, lac, gain, octaves, winWidth, winHeight)
	}
	if keyState[sdl.SCANCODE_G] != 0 {						//button "g" to effect gain
		gain = gain + 0.1*float32(mult)	
		makeNoise(pixels, frequency, lac, gain, octaves, winWidth, winHeight)
	}
	if keyState[sdl.SCANCODE_L] != 0 {						//button "l" to effect lactunarity

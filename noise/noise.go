...
func Fbm2(x, y, frequensy, lacunarity, gain float32, octaves int)float32{  //fractal brownian motion func, (lacunarity - reit of changing frequency; gain - rait of changen amptitude; octaves - how many iterations )
 ...
 //MakeNoise for making 2d slice of noise
func MakeNoise(noiseType NoiseType, frequency, lactunarity, gain float32, octaves, w, h int)(noise []float32, min, max float32){     
													  
	noise = make([]float32, w*h)			//for collecting all float 32 noise
	...
	for i := 0; i < numRoutines; i++ {					//creating loop where gorutens going to work
		go func(i int) {
			...
			innerMax := float32(-math.MaxFloat32)		//this two are needed to resolve gorutine racing
			start := i * batchSize
			...
			noise[j] = Turbulence(float32(x), float32(y), frequency, lactunarity, gain, octaves )  //append to slice of noise (can modify frequensies/lactunarity/gain/oktaves to change picture shown, like tuning old tv)
					...
					noise[j] = Fbm2(float32(x), float32(y), frequency, lactunarity, gain, octaves )
				}

				if noise[j] < innerMin{
				innerMin = noise[j]							//help us tu find min noise going thru slice
			}else if noise[j] > innerMax {
				innerMax = noise[j]							//help us to find max noise going thru slice
			}
		}
		mutex.Lock()
		if innerMin < min {
